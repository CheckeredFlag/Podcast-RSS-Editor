...
// Update config file
if (!empty($_POST['newXMLFile']) || !empty($_POST['newLanguage'])) {
    ...
    if (!empty($_POST["newLanguage"])) {
        $oldLangFile = $config['languageFile'];
        $newLangFile = preg_replace('/[^\w.\/ -]/', '', $_POST['newLanguage']);
        if ($oldLangFile !== $newLangFile && file_exists($newLangFile)) {
            $newConfig = str_replace($oldLangFile, $newLangFile, $newConfig);
        }
    }
    if ($newConfig !== $oldConfig) {
        saveConfig($configFile, $newConfig);
        header('Location: /?view=episodes&notf=1');
        exit;
    }
    header('Location: /?view=episodes');
    exit;
...
foreach ($xmlDoc->channel->item as $thisItem){
    ...
    $jumpto = $t >= 10 ? "#ep-{$t}" : "";	//~Set jump to in item list
    $highLightItem = (isset($_GET['ep']) && (int) $_GET['ep'] == $t) ? 'highlight' : '';
    $thisContent = <<<HTML
        <div class="item {$highLightItem}">
          ...
          <div class="item-btns">
            ...
            <a class="item-delete" href="?del=d{$t}" onclick="return confirm('{$lang[57]}{$thisTitle[$t]}{$lang[58]}')" data-balloon="{$lang[61]}" data-balloon-pos="left">
              <svg width="16" height="16" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
...
//~Process summaries
$latestDate  = strtotime(date('Y-m-d', strtotime($thisDate[0])));
...
for ($i = 0; $i < $t; $i++) {
    ...
    switch ($thisDateD[$i]){
        case 'Sun': $totalSunNo++; break;
        case 'Mon': $totalMonNo++; break;
        case 'Tue': $totalTueNo++; break;
        case 'Wed': $totalWedNo++; break;
        case 'Thu': $totalThuNo++; break;
        case 'Fri': $totalFriNo++; break;
        case 'Sat': $totalSatNo++; break;
        default:
          $totalSunNo++;
    }
    $N = $thisDateD[$i];
...
if (!empty($_GET['dupl'])) {
    ...
    $xmlDoc->asXML($xmlFileName);
    header('Location: /?ep=0&notf=4');
    exit;
}


//~Initialization
$panelTitle = $currentTitle = $currentDuration = '';
$currentHH = $currentMM = $currentSS = $currentHour = $currentMinute = $currentSecond = '';
$currentMonth = $currentDay = $currentYear = $currentLink = $currentAuthor = $currentImage = '';
$currentFile = $currentDesc = $currentDate = $currentDate2 = '';
$fileSize = $weekDay = '';

$averageCycle = $averageLineH = 0;
for ($z = 0; $z < 10; $z++) {
    $highestPoint[$z] = '';
    $durationChartH[$z] = '0';
}
...
if (isset($_POST['yy']) && $_POST['yy'] === 'yes' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'edit') {
        ...
        $xmlDoc->asXML($xmlFileName);
        header("Location: {$_SERVER['HTTP_REFERER']}&notf=2#ep-{$ep}");
        exit;
    }
}
...
elseif ($view === 'dashboard') {
    ...
    $totalWeekdayNo = [$totalSunNo, $totalMonNo, $totalTueNo, $totalWedNo, $totalThuNo, $totalFriNo, $totalSatNo];
    $maxWeekdayNo = max($totalWeekdayNo);
    $barHeight = 140;
    $w1 = 45;
    $j = 0;
    foreach (range(0,6) as $i) { $highestBar[$i] = ''; }
...
elseif ($view === 'settings') {
    ...
    $settingsContent = <<<HTML
        <form method="post" autocomplete="on" action="/?view=episodes&notf=1">
            <section>
                <label for="newXMLFile">{$lang[39]}</label>
                <select id="content" name="newXMLFile" class="settings-select right-in-3">

    HTML;
    foreach (glob('feeds/*.{rss,xml}', GLOB_BRACE) as $filename) {
        $basename = pathinfo($filename)['basename'];
        $selected = $xmlFileName === $filename ? 'selected="selected"' : '';
        $settingsContent .= "<option {$selected} value=\"{$filename}\">{$basename}</option>\n";
    }
    $settingsContent .= <<<HTML
                </select>
            </section>
    ...
    foreach (glob('language/*.php', GLOB_BRACE) as $languageFile) {
        $languageName = pathinfo($languageFile)['filename'];
        $selected = $config['languageFile'] === $languageFile ? 'selected="selected"' : '';
        $settingsContent .= "<option {$selected} value=\"{$languageFile}\">{$languageName}</option>";
    }
    $settingsContent .= <<<HTML
                </select>
            </section>
...
function loadConfig($file) {
    if (empty($_SESSION['config']) || empty($_SESSION['config']['xmlFile'])) {
        $_SESSION['config'] = include($file);
    }
    $_SESSION['config']['lang'] = include($_SESSION['config']['languageFile']);
...
<head>
  ...
  <title>Podcast RSS Editor - <?= htmlspecialchars($xmlDoc->channel->title) ?></title>
...
<section class="notification <?= $showNotification ?>">
  <div class="inner"><?= $notificationContent ?></div>
</section>

<nav>
  <a href="?view=episodes" class="logo"><?= $lang[0] ?></a>
  <ul>
    <li>
      <a href="?view=episodes" class="<?= $view === 'episodes' ? 'highlight' : '' ?>">
    ...
    <li>
      <a href="?view=dashboard" class="<?= $view === 'dashboard' ? 'highlight' : '' ?>">
        <svg xmlns="http://www.w3.org/2000/svg" width="60" height="60" viewBox="0 0 60 60">
    ...
    <li>
      <a href="?view=settings" class="<?= $view === 'settings' ? 'highlight' : '' ?>">
        <svg xmlns="http://www.w3.org/2000/svg" width="60" height="60" viewBox="0 0 60 60">
...
<article class="panel edit-panel">
  ...
  <div class="editor">
    ...
    <form action="/" method="post">
      ...
      <section class="edit-title right-in-2">
        <h3><?= $lang[7] ?> </h3>
        <span><input type="text" name="newTitle" value="<?= $currentTitle ?>" /></span>
      </section>
      <section class="edit-date right-in-3">
        <h3><?= $lang[8] ?> </h3>
        <span>
          <input type="text" name="newMonth" value="<?= $currentMonth ?>" /><label for="newMonth"><?= $lang[45] ?></label>
          <input type="text" name="newDay" value="<?= $currentDay ?>" /><label for="newDay"><?= $lang[46] ?></label>
          <input type="text" name="newYear" value="<?= $currentYear ?>" /><label for="newYear"><?= $lang[47] ?></label>
          <input type="text" name="newHour" value="<?= $currentHour ?>" /><label for="newHour"><?= $lang[48] ?></label>
          <input type="text" name="newMinute" value="<?= $currentMinute ?>" /><label for="newMinute"><?= $lang[49] ?></label>
          <input type="text" name="newSecond" value="<?= $currentSecond ?>" /><label for="newSecond"><?= $lang[50] ?></label>
        </span>
      </section>
      <section class="edit-duration right-in-4">
        <h3><?= $lang[9] ?> </h3>
        <span>
          <input type="text" name="newHH" value="<?= $currentHH ?>" /><label for="newHH"><?= $lang[51] ?></label>
          <input type="text" name="newMM" value="<?= $currentMM ?>" /><label for="newMM"><?= $lang[52] ?></label>
          <input type="text" name="newSS" value="<?= $currentSS ?>" /><label for="newSS"><?= $lang[53] ?></label>
        </span>
      </section>
      <section class="edit-link right-in-5">
        <h3>
          <?= $lang[10] ?>
          <em><a target="_blank" href="<?= $currentLink ?>"><?= $lang[72] ?></a> ⎋</em>
        </h3>
        <span><input type="text" name="newLink" value="<?= $currentLink ?>" /></span>
      </section>
      <section class="edit-author right-in-6"><h3><?= $lang[11] ?> </h3><span><input type="text" name="newAuthor" value="<?= $currentAuthor ?>" /></span></section>
      <section class="edit-image right-in-7">
        <h3>
          <?= $lang[12] ?>
          <em><a target="_blank" href="<?= $currentImage ?>"><?= $lang[72] ?></a> ⎋</em>
        </h3>
        <span><input type="text" name="newImage" value="<?= $currentImage ?>" /></span>
      </section>
      <section class="edit-audio right-in-8">
        <h3><?= $lang[13] ?> <em><a target="_blank" href="<?= $currentFile?>"><?= $lang[72] ?></a> ⎋</em></h3>
        <span><input type="text" name="newFile" value="<?= $currentFile ?>" /></span>
      </section>
      <section class="edit-desc right-in-9"><h3><?= $lang[14] ?> </h3><span><textarea name="newDesc"><?= $currentDesc ?></textarea></span></section>
    </form>
  </div>
</article>
<?php } ?>
...
<article class="dashboard panel-in <?= $showDashboard ?>">
  <section class="d-last-udpate">
    ...
    <strong class="scale-in-1"><?= $sinceLastUpdate ?></strong><?= $lang[15] ?><br />
    <i><?= $lang[16] ?><span><?= $averageCycle ?></span><?= $lang[17] ?></i>
  </section>
  ...
  <section class="d-total-episodes">
    ...
    <strong class="scale-in-2"><?= $t ?></strong><?= $lang[18] ?><br />
    <i><?= $lang[19] ?><span><?= $totalYears ?></span><?= $lang[20] ?></i>
  </section>
  ...
  <section class="d-total-hours">
    ...
    <strong class="scale-in-3"><?= $totalHours ?></strong><?= $lang[21] ?><br />
    <i><?= $lang[22] ?><span><?= number_format(($totalHours / 7.88),1) ?></span><?= $lang[23] ?></i>
  </section>
  ...
  <section class="d-file-size">
    ...
    <strong class="scale-in-4"><?= $fileSize ?></strong><?= $lang[24] ?><br />
    <i><?= $lang[25] ?><span><?= $totalShownoteLinks ?></span><?= $lang[26] ?></i>
  </section>
  ...
  <section class="d-host-num">
    ...
    <strong class="scale-in-5"><?= $totalHostsNo ?></strong><?= $lang[27] ?><br />
    <i><?= $lang[28] ?><span  data-balloon="<?= $lang[64] ?> <?= $hostsList ?>" data-balloon-pos="left" data-balloon-length="medium"><?= $mostHost ?></span><?= $lang[29] ?></i>
  </section>

  <section class="d-weekday">
    <div class="scale-in-1"><?= $weekDay ?></div>
  </section>
  ...
  <section class="d-duration">
    <div class="scale-in-6">
      <svg xmlns="http://www.w3.org/2000/svg" width="500" height="150" viewBox="-10 -10 490 140" class="">
        ...
        <path class="average-line" d="M 0 <?= $averageLineH ?>l 500 0" stroke-width="1" fill="none" />
        <path filter="url(#f3)" class="duration-chart" stroke="#ff2d77" stroke-width="3" fill="none" d="
          M10 <?= $durationChartH[0] ?>
          C30 <?= $durationChartH[0] ?> 40 <?= $durationChartH[1] ?> 60 <?= $durationChartH[1] ?>
          C80 <?= $durationChartH[1] ?> 90 <?= $durationChartH[2] ?> 110 <?= $durationChartH[2] ?>
          C130 <?= $durationChartH[2] ?> 140 <?= $durationChartH[3] ?> 160 <?= $durationChartH[3] ?>
          C180 <?= $durationChartH[3] ?> 190 <?= $durationChartH[4] ?> 210 <?= $durationChartH[4] ?>
          C230 <?= $durationChartH[4] ?> 240 <?= $durationChartH[5] ?> 260 <?= $durationChartH[5] ?>
          C280 <?= $durationChartH[5] ?> 290 <?= $durationChartH[6] ?> 310 <?= $durationChartH[6] ?>
          C330 <?= $durationChartH[6] ?> 340 <?= $durationChartH[7] ?> 360 <?= $durationChartH[7] ?>
          C380 <?= $durationChartH[7] ?> 390 <?= $durationChartH[8] ?> 410 <?= $durationChartH[8] ?>
          C430 <?= $durationChartH[8] ?> 440 <?= $durationChartH[9] ?> 460 <?= $durationChartH[9] ?>
        " />
        <g stroke="#ff2d77" stroke-width="3" fill="#232733">
          <circle class="point-1 <?= $highestPoint[0] ?>" cx="10" cy="<?= $durationChartH[0] ?>" r="4" />
          <circle class="point-2 <?= $highestPoint[1] ?>" cx="60" cy="<?= $durationChartH[1] ?>" r="4" />
          <circle class="point-3 <?= $highestPoint[2] ?>" cx="110" cy="<?= $durationChartH[2] ?>" r="4" />
          <circle class="point-4 <?= $highestPoint[3] ?>" cx="160" cy="<?= $durationChartH[3] ?>" r="4" />
          <circle class="point-5 <?= $highestPoint[4] ?>" cx="210" cy="<?= $durationChartH[4] ?>" r="4" />
          <circle class="point-6 <?= $highestPoint[5] ?>" cx="260" cy="<?= $durationChartH[5] ?>" r="4" />
          <circle class="point-7 <?= $highestPoint[6] ?>" cx="310" cy="<?= $durationChartH[6] ?>" r="4" />
          <circle class="point-8 <?= $highestPoint[7] ?>" cx="360" cy="<?= $durationChartH[7] ?>" r="4" />
          <circle class="point-9 <?= $highestPoint[8] ?>" cx="410" cy="<?= $durationChartH[8] ?>" r="4" />
          <circle class="point-10 <?= $highestPoint[9] ?>" cx="460" cy="<?= $durationChartH[9] ?>" r="4" />
        </g>
        <g>
          <text x="0" y="<?= $durationChartH[0] + 20 ?>" class="duration-chart-text"><?= $thisMinutes[0] ?></text>
          <text x="50" y="<?= $durationChartH[1] + 20 ?>" class="duration-chart-text"><?= $thisMinutes[1] ?></text>
          <text x="100" y="<?= $durationChartH[2] + 20 ?>" class="duration-chart-text"><?= $thisMinutes[2] ?></text>
          <text x="150" y="<?= $durationChartH[3] + 20 ?>" class="duration-chart-text"><?= $thisMinutes[3] ?></text>
          <text x="200" y="<?= $durationChartH[4] + 20 ?>" class="duration-chart-text"><?= $thisMinutes[4] ?></text>
          <text x="250" y="<?= $durationChartH[5] + 20 ?>" class="duration-chart-text"><?= $thisMinutes[5] ?></text>
          <text x="300" y="<?= $durationChartH[6] + 20 ?>" class="duration-chart-text"><?= $thisMinutes[6] ?></text>
          <text x="350" y="<?= $durationChartH[7] + 20 ?>" class="duration-chart-text"><?= $thisMinutes[7] ?></text>
          <text x="400" y="<?= $durationChartH[8] + 20 ?>" class="duration-chart-text"><?= $thisMinutes[8] ?></text>
          <text x="450" y="<?= $durationChartH[9] + 20 ?>" class="duration-chart-text"><?= $thisMinutes[9] ?></text>
        </g>
      </svg>
      <br>
      <label><?= $lang[37] ?><span><?= $averageMinutes ?> </span><?= $lang[38] ?></label>
    </div>
  </section>
</article>
<?php } ?>
...
<article class="settings panel-in <?= $showSettings ?>">
